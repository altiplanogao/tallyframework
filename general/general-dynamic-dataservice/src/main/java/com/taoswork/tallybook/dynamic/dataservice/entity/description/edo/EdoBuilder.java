...
//        }
        ...
        for (Map.Entry<String, FieldMetadata> fieldMetadataEntry : fieldMetadataMap.entrySet()){
            ...
            classEdo.addField(fieldEdo);

            OrderedName fieldName = fieldEdo.makeOrderedName();

            if (fieldMetadata.showOnGrid() || fieldMetadata.isId()) {
                classEdo.addGridFieldName(fieldName);
            }

            if(fieldMetadata.isId()){
                classEdo.setIdField(fieldEdo.getName());
            }

            {
                ...
                groupEdo.addFieldName(fieldName);
            }
        }

        FieldEdo primarySearchField = classEdo.calculatePrimaryGridSearchField();
        if(primarySearchField != null){
            classEdo.setPrimarySearchField(primarySearchField.getName());
        }
    ...
    public static void makeGridFields(ClassEdo classEdo){
        for (TabEdo tabEdo : classEdo.getTabs()){
            for(GroupEdo groupEdo : tabEdo.getGroups()){
                for(FieldEdo fieldEdo : groupEdo.getFieldNames()){
                    fieldEdo.
                }
            }
        }
